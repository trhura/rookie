...
static void update_combo_boxes (DownloadDialog *dialog);
//static void update_active_category_and_backend ();
...
download_dialog_init (DownloadDialog *object)
{
	...
	g_downloadable_backends_foreach ((GFunc)backend_combo_append_text,
									 object->priv->backend);
	categories_foreach_category  ((GFunc)category_combo_append_text,
								  object->priv->category);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (object->priv->start_downloading),
...
static void
update_combo_boxes (DownloadDialog *dialog)
{
	if (dialog->priv->first_time == TRUE) {
...
static void
on_url_change (GtkEditable * editable, gpointer data)
{
...
static void
on_save_as_change (GtkEditable * editable, gpointer data)
{
...
GDownloadableBackend *download_dialog_get_backend (DownloadDialog *dialog)
{
	...
	gchar *backend_name;
	backend_name = gtk_combo_box_text_get_active_text (GTK_COMBO_BOX_TEXT(dialog->priv->backend));
